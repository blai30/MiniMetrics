...
public static class MetricFormatting
{
    ...
    public static string FormatTempValue(double celsius) => Math.Round(celsius).ToString("0", CultureInfo.InvariantCulture);

    public static string FormatPower(double watts) => $"{Math.Round(watts).ToString("0", CultureInfo.InvariantCulture)} W";
}
